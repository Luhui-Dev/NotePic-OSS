...
- 4.1 各格式的压缩参数（JPEG 质量/渐进式、PNG 无损 vs 量化、WebP 编码方式）——目前 CLI 和 Obsidian 在 PNG 处理以及“未知格式带 alpha 通道”的回退路径上存在分歧。已记录为待办项（§6）；修复时无需跟随协议版本号变化。
...
- PNG 压缩语义不同（CLI：始终无损；Obsidian：quality≥100 才无损，否则量化）。
